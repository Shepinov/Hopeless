...
[ExecuteInEditMode]
public class SearchCover : MonoBehaviour {

public Vector3 coverPostion; //the final coverposition
public Vector3 eyePosition; //the eye position modifier
public float amountOfRays; //the amount of checking rays
public float fieldOfRays; //the field of rays in degrees, like a FOV
public string tagToAvoid; //the tag of the object that needs to be covered from
public float distanceToCheck; //the distance to check the rays
public bool debugActive = false; //whether to debug or not
public float trueCoverTest = 10f; //this heavily influences performance: the lower the better the performace, but worse cover predictions
...
private GameObject avoidGameobject; //the internal gameobject to avoid
...
/// <summary>
/// Finds the cover postion at the relative eye level.
/// </summary>
/// <param name="eyeLevel">Eye level.</param>
/// <param name="halfCover">If set to <c>true</c> find the best half cover.</param>
public void FindCoverPostion(Vector3 eyeLevel, bool halfCover)
{
		...
		switch( coverSystemToUse )
		{
			...
			case CoverSystem.Predefined: selectionArray = predefinedPoints; CalculateBestCover(halfCover); break;
		...
		{
			objectToAvoidExists = true;
			avoidGameobject = GetComponent<AIStateManager>().sight.GetComponent<Sight>().objectToSeek;
		}
		else
		{
			objectToAvoidExists = false;
		...
		CalculateBestCover( halfCover);
	}
	
	
	
//calculate the best cover from the info gathered
	void CalculateBestCover(bool halfCover)
	{
	...
	Vector3 FindHalfCover( Vector3 self, Vector3 player)
	{
		...
		while(testID < selectionArray.Count-1)
		{
			...
			if( Physics.Raycast( (selectionArray[testID] + eyePosition), testVector, out hit, testVector.magnitude) )
			{
				...
				selectionArray.Remove( selectionArray[testID] );
		...
		while(testID < selectionArray.Count-1)
		{
			...
			RaycastHit[] hits;
			hits = Physics.RaycastAll(selectionArray[testID], player-selectionArray[testID]);
		...
		while(testID < selectionArray.Count-1)
		{
			...
			if(Vector3.Distance( self, selectionArray[testID]) < smallestDistance)
			{
