...
public class IKLimb: MonoBehaviour {
	...
	void CalculateIK(){
		...
		if( float.IsNaN(upperArmAxisCorrection.transform.localRotation.eulerAngles.x - ikAngle) == false &&
		   float.IsNaN(upperArmAxisCorrection.transform.localRotation.eulerAngles.y) == false &&
		   float.IsNaN(upperArmAxisCorrection.transform.localRotation.eulerAngles.z) == false)
		   {
			
			upperArmAxisCorrection.transform.localRotation = Quaternion.Euler( new Vector3( upperArmAxisCorrection.transform.localRotation.eulerAngles.x - ikAngle, upperArmAxisCorrection.transform.localRotation.eulerAngles.y, 
		                                                                               upperArmAxisCorrection.transform.localRotation.eulerAngles.z) );
		}                                                                                                                                                                                                                                                                                                                                
		...
		//little errors accumilate
		if(elbowTarget.GetComponent<ElbowTarget>().elbowTargetType == ArmType.Left)
		{
			Vector3 newRot = forearm.rotation.eulerAngles - new Vector3( 0, 180f, 180f );
			forearm.rotation = Quaternion.Euler( newRot );
		}
